...
def from_to(sl, tl, content):
    ...
    with open('./translate.js', 'r', encoding='UTF-8') as file:
        js_text = file.read()
        # 编译加载js字符串
        js = execjs.compile(js_text)
        sign = js.call("e", content, gtk)
...
def baidu_translate(params):
    ...
    Returns:
        json: {result: 翻译后的内容}
    """
    return from_to(read_lang(params['from']), read_lang(params['to']), params['content'])
